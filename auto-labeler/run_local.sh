...
if ! check_service_running "mongodb" 27017; then
    forward_and_catch "auto-labeler-mongo" 27017 27017 "$NAMESPACE" &
fi

export MONGODB_PASSWORD=$(kubectl --context=$CONTEXT --namespace=$NAMESPACE get secret auto-labeler-mongo -o yaml | yq .data.password | base64 -d)
export MONGODB_USER=$(kubectl --context=$CONTEXT --namespace=$NAMESPACE get secret auto-labeler-mongo -o yaml | yq .data.username | base64 -d)
...
echo "Starting ..."
export MONGO_URI="mongodb://${MONGODB_USER}:${MONGODB_PASSWORD}@localhost:27017"
export MONGO_DATABASE="auto_label_labeler"
export BASE_URL="/"
